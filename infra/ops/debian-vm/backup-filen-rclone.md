...
Weekly rclone upload of all TrueNAS backup archives to Filen Cloud using client-side encryption. This is the off-site disaster recovery copy.
...
Reads PBS backup archives from the TrueNAS NFS mount and syncs them to Filen Cloud via rclone with the `filen-crypt` encrypted remote. The NVMe application data is not uploaded separately — it is already captured inside the Debian VM PBS backup archive stored on TrueNAS, so syncing TrueNAS is sufficient to cover everything.
...
> ⚠️ **Missing improvement:** This script uses plain `rclone sync` without `--backup-dir`. If a PBS archive is deleted from TrueNAS, it will be permanently deleted from Filen Cloud on the next run with no recovery path. Consider adding `--backup-dir "filen-crypt:backups-deleted/$(date +%F)"`. See `known-issues.md`.

---

## rclone Config Required

The following remotes must exist in `/root/.config/rclone/rclone.conf` on the Debian VM:
...
| File | Purpose |
|---|---|
| `/mnt/truenas` | Source — TrueNAS NFS mount (read-only) |
