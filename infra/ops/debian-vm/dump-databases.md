...
Nightly database dump script. Exports all application databases from their Docker containers to the TrueNAS NFS mount so they are picked up by the weekly rclone upload to Filen Cloud.
...
Dumps are written directly to TrueNAS NFS so the rclone upload on Sunday automatically includes them. The Debian VM never holds a persistent local copy.
...
| File | Purpose |
|---|---|
| `/mnt/truenas/db-dumps/` | Dump destination on TrueNAS NFS |
...
- TrueNAS NFS mounted at `/mnt/truenas` (read-write for this path)
