...
Quarterly point-in-time system snapshot of the Proxmox host. Captures hardware state, ZFS pool status, NVMe SMART data, kernel events, and VM status into a single text file for incident analysis and long-term review.
...
| HDD health note | Reminder that HDDs are passed through to TrueNAS — check there |
...
# 5 HDDs (3x4TB + 2x8TB) via LSI SAS 9300-16i passed through to TrueNAS VM.
