...
Proxmox Host
192.168.178.127	TrueNAS VM
192.168.178.139	PBS Mini PC
192.168.178.142	Debian VM
192.168.178.141
...
⦁	Step 1 — TrueNAS (192.168.178.139)  — NFS exports and healthcheck user
⦁	Step 2 — PBS Mini PC (192.168.178.142)  — NFS mount and marker script
⦁	Step 3 — Proxmox Host (192.168.178.127)  — NVMe health, forensic, analytics scripts
⦁	Step 4 — Debian VM (192.168.178.141)  — Main monitoring and backup scripts
⦁	Step 5 — Verification  — Confirm everything is connected and working

⚠  All commands in this guide are run as root unless stated otherwise. Use sudo -i or log in as root before starting.

Script Inventory
Complete list of all scripts and where they are installed.
...
1.	Add this single line:
healthcheck ALL=(root) NOPASSWD: /sbin/zpool list *
1.	Save and verify syntax:
...
⚠  The same dataset path is mounted at different local paths on each client: /mnt/truenas-backup on PBS (RW) and /mnt/truenas on Debian VM (RO). Both resolve to the same TrueNAS export.

1.4  Add the Debian VM SSH public key
This step is completed after Step 4.1 where the key is generated on the Debian VM. Come back here then.

1.	After generating the key on the Debian VM, copy the public key:
cat /root/.ssh/id_ed25519_truenas.pub
1.	On TrueNAS:  Credentials → Local Users → healthcheck → Edit
2.	Paste the public key into the SSH Public Keys field
3.	Save
...
1.	Set /etc/aliases so root mail goes to your real address:
...
1.	Confirm marker was written:
cat /mnt/truenas-backup/.pbs-last-success.txt

2.5  Install crontab entry
1.	Open crontab:
crontab -e
1.	Add this line — runs 30 minutes after backup window closes at ~04:00:
30 4 * * *   /usr/local/bin/pbs-write-marker.sh >> /var/log/pbs-marker.log 2>&1
⚠  Adjust the time to be at least 30 minutes after your PBS backup job typically finishes. Check PBS job history to confirm the typical finish time.
...
⚠  If you want to change any threshold (e.g. capacity warning level), edit /root/forensic-config.sh. All four analytics scripts pick up the change automatically.
...
⚠  If it fails, read the error message. Most common causes: nvme-cli not installed, /var/lib/docker not mounted, rpool name different from 'rpool'.

3.7  Run first forensic dump and integrity chain
1.	Run forensic dump — this also initialises the integrity chain:
/usr/local/bin/forensic-dump.sh
1.	Confirm the dump was written:
ls -la /root/forensic-history/dumps/
1.	Confirm the integrity chain was created with append-only flag:
lsattr /root/forensic-history/dumps/integrity-chain.txt
1.	You should see the 'a' flag:
-----a-------- /root/forensic-history/dumps/integrity-chain.txt
1.	If the 'a' flag is missing, set it manually:
chattr +a /root/forensic-history/dumps/integrity-chain.txt

3.8  Install Proxmox crontab entries
1.	Open crontab:
crontab -e
1.	Add all Proxmox entries from crontab-reference.txt:
...
vi /etc/chrony.conf
1.	Add or replace the server line with:
server 192.168.178.127 iburst prefer
makestep 1.0 3
1.	Restart chrony:
systemctl restart chronyd
1.	Verify it is syncing from Proxmox (look for * next to the Proxmox IP):
chronyc sources
1.	Confirm tracking output shows Proxmox as the reference:
chronyc tracking
...
1.	Mount it:
mount /mnt/truenas
1.	Verify it is responsive:
ls /mnt/truenas

4.5  Generate SSH key for TrueNAS healthcheck user
1.	Generate the key:
ssh-keygen -t ed25519 -f /root/.ssh/id_ed25519_truenas -N ""
1.	Accept TrueNAS host key into known_hosts:
ssh-keyscan 192.168.178.139 >> /root/.ssh/known_hosts
1.	Display the public key — copy this for Step 1.4:
cat /root/.ssh/id_ed25519_truenas.pub
1.	Now complete Step 1.4 on TrueNAS (add this key to healthcheck user)
2.	Then verify the SSH connection works:
...
1.	Verify the inhibit file was created:
cat /var/run/backup_inhibit
1.	Expected output shows the reboot timestamp and reason:
...
1.	If all checks pass, the inhibit file is cleared:
ls /var/run/backup_inhibit   # should return: No such file or directory
1.	If it fails, read the output and fix the reported issue before continuing

⚠  The most common first-run failures are: NTP not yet syncing from Proxmox (wait 1-2 minutes after chrony restart), TrueNAS SSH not yet configured (complete Step 1.4 first), PBS marker not yet written (run pbs-write-marker.sh on PBS host manually), Filen marker not yet written (run backup-filen-rclone.sh manually once).

4.10  Install Debian VM crontab entries
1.	Open crontab:
crontab -e
1.	Add all Debian VM entries:
...
/usr/local/bin/dump-databases.sh
1.	Confirm dumps were written to TrueNAS:
ls -la /mnt/truenas/db-dumps/
1.	Expected files:  pg-paperless-YYYY-MM-DD.dump,  pg-immich-YYYY-MM-DD.dump,  redis-paperless-YYYY-MM-DD.rdb
...
/usr/local/bin/backup-filen-rclone.sh
1.	Confirm the marker was written:
cat /var/log/filen-last-success.txt

5.5  Confirm daily report sends correctly
1.	Run manually:
/usr/local/bin/homelab-daily-report.sh
1.	Check your email inbox for:  [HOMELAB DAILY] OK | ...
2.	Check the log:
...
reboot
1.	After reboot, confirm inhibit was set by the boot service:
cat /var/run/backup_inhibit   # should show REBOOT + timestamp
1.	Wait up to 5 minutes for monitor-backups.sh cron to run, then:
ls /var/run/backup_inhibit   # should now be gone
1.	Check tier0.log to confirm it ran and passed:
...
sudo /usr/local/bin/test-restore.sh 2>&1 | tee /var/log/restore-test-$(date +%F).log
1.	Review the output — all checks must show [PASS]
2.	If any check fails, investigate and fix before relying on that backup
...
1.	All entries should show [OK]
2.	MISSING entries after a prune are expected — that is normal and correct

Email subjects reference
...
Pending Items (to complete later)
...
nut-scanner -U
1.	Configure NUT server on Proxmox:  /etc/nut/nut.conf,  ups.conf,  upsd.conf,  upsd.users,  upsmon.conf
2.	Start NUT services:
systemctl enable --now nut-server nut-monitor
1.	Configure NUT client on Debian VM:  /etc/nut/nut.conf (MODE=netclient),  upsmon.conf
2.	In monitor-backups.sh: uncomment the blocking UPS check and set UPS_NAME
3.	In homelab-daily-report.sh: the UPS section activates automatically once upsc works

⚠  Until this is complete, you will receive periodic [HOMELAB UPS PENDING] emails from monitor-backups.sh. These are informational only and do not indicate a system failure.
