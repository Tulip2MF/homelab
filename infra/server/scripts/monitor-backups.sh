...
#
# NETWORK MAP:
#   Proxmox host      192.168.178.127  (NTP master, hypervisor)
#   TrueNAS VM        192.168.178.139  (backup store, NFS server)
#   PBS Mini PC       192.168.178.142  (nightly VM backups -> TrueNAS)
#   Debian VM         192.168.178.141  (THIS HOST -- apps + rclone)
#
# ARCHITECTURE:
...
#
#   Proxmox (192.168.178.127) = NTP master. All VMs MUST sync from it only.
#
...
#   4. TrueNAS NFS unresponsive
#   5. TrueNAS ZFS pool not ONLINE
#   6. TrueNAS at/near capacity
#   7. PBS backup not completed within RPO window
#   8. rclone Filen upload not completed within window
#
# SETUP REQUIRED (one-time):
#   1. On TrueNAS: create user 'healthcheck', add to group with sudo access
#      to 'zpool list' only. Add sudoers line:
#        healthcheck ALL=(root) NOPASSWD: /sbin/zpool list *
#   2. On Debian VM: ssh-keygen -t ed25519 -f /root/.ssh/id_ed25519_truenas
#      ssh-copy-id -i /root/.ssh/id_ed25519_truenas.pub healthcheck@192.168.178.139
#   3. Test: ssh -i /root/.ssh/id_ed25519_truenas healthcheck@192.168.178.139 \
#            "sudo zpool list -H -o health"
#   4. Add Proxmox as NTP source in /etc/chrony.conf on Debian VM:
#        server 192.168.178.127 iburst prefer
#        makestep 1.0 3
#   5. Install NUT client on Debian VM and configure ups.conf
#      pointing to Proxmox NUT server.
#   6. Ensure /mnt/truenas is mounted (add to /etc/fstab):
#        192.168.178.139:/mnt/tank/backups/proxmox-backups  /mnt/truenas  nfs  ro,soft,timeo=30  0 0
#   7. Ensure /var/lib/docker is mounted (Proxmox-assigned NVMe partition).
#
# Cron (Debian VM) -- interval must match HEARTBEAT_MAX_AGE below:
...
# PBS marker written by pbs-write-marker.sh on PBS host to TrueNAS NFS.
...
# ─────────────────────────────────────────────────────────────────────────────
### 2. POWER (UPS)
# ⚠ PENDING SETUP: UPS USB cable not yet connected to Proxmox (192.168.178.127).
# NUT is installed on Proxmox but not configured -- no device detected yet.
#
# TO ACTIVATE:
#   1. Connect UPS USB cable to Proxmox host
#   2. Run: nut-scanner -U  (on Proxmox) to identify driver + port
#   3. Generate NUT config files (nut.conf, ups.conf, upsd.conf, upsd.users,
#      upsmon.conf) for Proxmox master and Debian VM slave
#   4. Install nut-client on Debian VM, configure upsmon.conf pointing to
#      192.168.178.127
...
#   [ -z "$UPS_STATUS" ] && fail "UPS status unreadable -- NUT on Proxmox (192.168.178.127) unreachable"
...
if ! command -v upsc >/dev/null 2>&1; then
    ...
    echo "UPS check PENDING: upsc not installed on Debian VM" \
        | mail -s "[HOMELAB UPS PENDING] Install nut-client on shipyard" root 2>/dev/null || true
elif ! upsc -l 2>/dev/null | grep -q .; then
    # NUT not reachable or no devices -- non-blocking warn (setup in progress)
    if [ ! -f "$UPS_WARN_SENT" ]; then
        echo "[TIER-0] WARN: UPS/NUT not configured on Proxmox (192.168.178.127) -- pending USB connection" >&2
    ...
    # NUT is reachable -- but UPS_NAME not yet confirmed, warn only
    echo "[TIER-0] WARN: NUT reachable but UPS_NAME not confirmed in this script -- activate blocking check" >&2
    rm -f "$UPS_WARN_SENT"
fi
# NOTE: UPS check is NON-BLOCKING until setup is complete.
# When activated, a UPS failure WILL set inhibit and block all backups."

# ─────────────────────────────────────────────────────────────────────────────
### 3. TIME -- must sync FROM Proxmox NTP master (192.168.178.127)
# If this VM were syncing from an external NTP server independently,
# it could silently drift away from the rest of the infrastructure,
# breaking PBS snapshot ordering and backup timestamps.
...
# Active source (*) must be Proxmox IP specifically
...
# Docker container volumes live here. If unmounted or full, apps are dead.
...
# TrueNAS holds all PBS archives. Debian mounts it via NFS for rclone.
# Blocking because:
#   - NFS hung    -> rclone hangs or fails -> cloud backup silently broken
#   - Pool not ONLINE -> archives may be incomplete or corrupt
#   - Capacity critical -> PBS write failures likely in progress

# 5a. NFS responsiveness -- catches D-state hung mounts
timeout 10 ls "$NFS_MOUNT" >/dev/null 2>&1 || \
    fail "TrueNAS NFS ${NFS_MOUNT} unresponsive -- rclone source unavailable"
...
[ -z "$POOL_STATE" ] && fail "Cannot reach TrueNAS via SSH (${TRUENAS_HOST}) -- pool state unknown"
[ "$POOL_STATE" != "ONLINE" ] && fail "TrueNAS ZFS pool is ${POOL_STATE} -- archives may be degraded"

# 5c. TrueNAS capacity
TRUENAS_USED=$(df -P "$NFS_MOUNT" 2>/dev/null \
    | awk 'NR==2 {gsub(/%/,"",$5); print int($5)}') || true
[ -z "$TRUENAS_USED" ] && fail "Cannot read TrueNAS capacity via NFS"
[ "$TRUENAS_USED" -ge "$CRIT_CAPACITY" ] && \
    fail "TrueNAS CRITICAL capacity (${TRUENAS_USED}%) -- PBS writes likely failing"
[ "$TRUENAS_USED" -ge "$WARN_CAPACITY" ] && fail "TrueNAS WARNING capacity (${TRUENAS_USED}%)"
[ "$TRUENAS_USED" -ge "$MAX_CAPACITY_OK" ] && fail "TrueNAS DEGRADED capacity (${TRUENAS_USED}%)"

# ─────────────────────────────────────────────────────────────────────────────
### 6. PBS BACKUP RECENCY
# PBS Mini PC (192.168.178.142) runs nightly backups and writes this marker to TrueNAS
# NFS via pbs-write-marker.sh. If the marker is missing or stale, rclone
# would upload outdated archives to Filen.
[ ! -f "$PBS_MARKER" ] && \
...
# Written by backup-filen-rclone.sh after successful rclone upload.
[ ! -f "$FILEN_MARKER" ] && \
...
# If inhibit is still set after all checks passed, refuse to clear it silently.
[ -f "$INHIBIT" ] && \
    fail "Inhibit still present after all checks -- operator must clear manually"

# ─────────────────────────────────────────────────────────────────────────────
### 9. SUCCESS -- heartbeat is the LAST write. Silence = FAILURE.
pass
