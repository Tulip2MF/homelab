...
| 4. Harbour (TrueNAS) Storage | NFS mount, ZFS pool health, capacity |
...
if [ -z "$POOL_STATE" ]; then
    echo "  ZFS pool:     UNKNOWN -- cannot reach Harbour (TrueNAS) via SSH"
    ISSUES=$(( ISSUES+1 )); OVERALL="ACTION REQUIRED"
...
if [ -f "$NVME_HEALTH_LOG" ]; then
    ...
    echo "  SMART/rpool:  check-nvme-health.sh runs on Main Server — Proxmox (192.168.178.127)"
