...
5. **TrueNAS** — NFS responsive, ZFS pool ONLINE, capacity within limits
6. **PBS backup recency** — marker file on TrueNAS must be within 25-hour RPO window
...
Applies to both TrueNAS NFS and NVMe partition.
...
| `/mnt/truenas/.pbs-last-success.txt` | Written by `pbs-write-marker.sh` on PBS host |
...
- `pbs-write-marker.sh` running on PBS host and writing to TrueNAS NFS
...
```bash
# 1. Generate SSH key for TrueNAS healthcheck user
ssh-keygen -t ed25519 -f /root/.ssh/id_ed25519_truenas -N ""

# 2. Accept TrueNAS host key
ssh-keyscan 192.168.178.139 >> /root/.ssh/known_hosts

# 3. Add public key to TrueNAS healthcheck user (via TrueNAS web UI)
