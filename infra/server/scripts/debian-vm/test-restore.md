...
2. PBS archive files exist on TrueNAS NFS and are non-zero size
...
| 2. TrueNAS NFS integrity | Mount responsive, PBS `.fidx` / `.blob` files exist and non-zero |
...
| File | Purpose |
|---|---|
| `/mnt/truenas` | TrueNAS NFS — checked for PBS archive files |
