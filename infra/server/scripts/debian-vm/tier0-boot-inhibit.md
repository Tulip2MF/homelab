...
After a reboot, system state is unknown. The previous heartbeat is stale, NFS mounts may not yet be up, and the PBS marker on Harbour (TrueNAS) reflects pre-reboot conditions. If backups were allowed to run immediately, they could run against an unverified system.
